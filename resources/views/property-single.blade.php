...
@push('head')
    ...
    <style>
        ...
        /* POI nearest list: keep each name to a single line (ellipsis); distance stays right */
        #poiNearestList .d-flex>span:not(.text-nowrap) {
            flex: 1;
            min-width: 0;
            overflow: hidden;
        ...
        .service-slider-zone--prev {
            left: 0;
            border-top-left-radius: 10px;
            border-bottom-left-radius: 10px;
        }

        .service-slider-zone--next {
            right: 0;
            border-top-right-radius: 10px;
            border-bottom-right-radius: 10px;
        }

        .service-slider-zone--prev:hover {
            background: linear-gradient(to right, rgba(0, 0, 0, .35), transparent);
        }

        .service-slider-zone--next:hover {
        ...
        .service-slider-zone--prev::after {
            background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23222'><path d='M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12z'/></svg>");
        }

        .service-slider-zone--next::after {
            background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23222'><path d='M8.59 16.59 10 18l6-6-6-6-1.41 1.41L13.17 12z'/></svg>");
        }

        .service-slider-zone:hover::after {
            opacity: 1;
       ...
       `.slider-card .slider-nav-thumbnails .slick-slide` rule in style.css. */
        .slider-card .slider-nav-thumbnails {
            margin: 16px 0 24px;
            /* reserve space between main image and section below */
        }

        .slider-card .slider-nav-thumbnails .slick-list {
            overflow: hidden;
            /* keep slick clones outside the visible area */
        }

        .slider-card .slider-nav-thumbnails .slick-slide {
            margin: 0 6px;
            height: auto;
            /* let the thumbnail scale by aspect-ratio, not a fixed px height */
            cursor: pointer;
        ...
        .slider-card .slider-nav-thumbnails .slick-slide .slide-img {
            ...
            width: 100%;
            aspect-ratio: 16 / 10;
            /* height tracks the slick-set width across all breakpoints */
            height: auto;
            border-radius: 8px;
            overflow: hidden;
       ...
       Prevents the "stacked images → carousel" jump (FOUC). */
        ...
        .slider-card.gallery-loading .slide-part {
            aspect-ratio: 16 / 10;
        }

        /* definite box while loading */
        .slider-card.gallery-loading .service-slider {
            visibility: hidden;
        }

        /* hidden but measurable for slick */
        .slider-card.gallery-loading .service-slider-zone {
            display: none;
        }
        ...
        .slider-card.gallery-loading .slider-nav-thumbnails {
            display: none;
        }
...
@section('content')
    ...
    @php
        ...
        $featureIconMap = [
            ...
            'gym' => 'fitness_center',
            'security' => 'security',
            'loadingdock' => 'local_shipping',
            'panoramicwindows' => 'panorama',
            'sauna' => 'hot_tub',
            'fireplace' => 'local_fire_department',
            'gazebo' => 'deck',
            'barbecuearea' => 'outdoor_grill',
            'sportscourt' => 'sports',
        ];
        $applianceIconMap = [
            ...
            'vacuumcleaner' => 'cleaning_services',
        ];
        $utilityIconMap = [
            'electricity' => 'bolt',
            'water' => 'water_drop',
    ...
    <div class="page-wrapper" data-slug="{{ $property['slug'] ?? '' }}">
        ...
        <div class="buy-details-header-item" @if($primaryImage)
                    ...
                    <div class="row align-items-center text-center position-relative z-1">
                        <div class="col-xl-8">
                            <div class="d-flex align-center gap-2 mb-2 flex-wrap">
                                ...
                                <span
                                    class="badge bg-primary">{{ __($__ptKey) !== $__ptKey ? __($__ptKey) : ($property['propertyType'] ?? '') }}</span>
                                ...
                                <span class="text-white-50 d-flex align-items-center gap-1 fs-14" id="view-count-wrap">
                                    ...
                                    <span id="property-view-count"
                                        data-count="{{ $property['viewCount'] ?? 0 }}">{{ $property['viewCount'] ?? 0 }}</span>
                                    {{ __('property.views') }}
                                </span>
                            </div>
                            ...
                            <div class="d-flex align-items-center gap-2 flex-wrap gap-1 mb-xl-0 mb-4">
                                @if(!empty($property['createdAt']))
                                    ...
                                    <p class="fs-14 mb-0 pt-2 text-white">{{ __('property-single.last_updated_on') }}
                                        {{ $__d->format('d') }} {{ __('property.' . strtolower($__d->format('M'))) }}
                                        {{ $__d->format('Y') }}</p>
                                @endif
                            </div>
                        </div>
                        <div class="col-xl-4 d-flex d-xl-block flex-wrap align-items-center gap-3">
                            <div
                                class="breadcrumb-icons d-flex align-items-center justify-content-xl-end justify-content-start gap-2 mb-xl-4 mt-xl-0 my-3">
                                <a href="javascript:void(0);" class="compare-btn" data-slug="{{ $property['slug'] ?? '' }}"
                                    aria-label="{{ __('header.compare') }}" title="{{ __('header.compare') }}"><x-icon
                                        name="balance" size="20" /></a>
                                <a href="javascript:void(0);" class="favourite" data-slug="{{ $property['slug'] ?? '' }}"
                                    aria-label="{{ __('property-single.favorite') }}"><x-icon name="favorite_border"
                                        size="20" /></a>
                                <a href="javascript:void(0);" class="share-btn" id="sharePropertyBtn"><x-icon name="share"
                                        size="20" /></a>
                            </div>
                            ...
                            <div class="price-alt-currencies text-xl-end text-start mt-1 js-price-alt"
                                ...
                                data-currency="{{ $property['currency'] ?? display_currency() }}" @unless(count($altPrices))
                                style="display:none" @endunless>
                                @foreach($altPrices as $ap)
                                    <span class="price-alt-currency">{{ $ap['formatted'] }}</span>@unless($loop->last)<span
                                    class="price-alt-sep">·</span>@endunless
                                @endforeach
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-xl-8">
                            ...
                            <div class="slider-card service-slider-card mb-4 gallery-loading">
                                <div class="slide-part mb-4 service-slider-wrap">
                                    ...
                                    <div class="slider service-slider">
                                        @foreach($mediaImages as $img)
                                            <div class="service-img-wrap">
                                                <img src="{{ $img['url'] }}" class="img-fluid" @if($loop->first)
                                    ...
                                    @if($mediaImages->count() > 1)
                                        <button type="button" class="service-slider-zone service-slider-zone--prev"
                                            aria-label="Previous image"></button>
                                        <button type="button" class="service-slider-zone service-slider-zone--next"
                                ...
                                @if($mediaImages->count() > 1)
                                    ...
                                    <div class="slider slider-nav-thumbnails text-center">
                                        @foreach($mediaImages as $img)
                                            <div class="slide-img"><img src="{{ $img['thumbnailUrl'] ?? $img['url'] }}"
                                                    class="img-fluid" loading="lazy" alt="{{ $property['title'] ?? '' }}"
                                                    onerror="this.onerror=null;this.style.background='#f0f0f0';this.style.minHeight='80px';">
                                            </div>
                                        @endforeach
                                    </div>
                                @endif
                        ...
                        @elseif($property['primaryImageUrl'] ?? null)
                            <div class="mb-4">
                                <img src="{{ $property['primaryImageUrl'] }}" class="img-fluid rounded w-100"
                                    alt="{{ $property['title'] ?? '' }}"
                                    onerror="this.onerror=null;this.parentElement.innerHTML='<div class=\'img-placeholder\' style=\'height:400px;border-radius:8px;\'>832 x 472</div>';">
                            </div>
                        @else
                            <div class="mb-4">
                        ...
                        <div class="accordion accordions-items-seperate">
                            ...
                            <div class="accordion-item">
                                <div class="accordion-header">
                                    <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#accordion-2" aria-expanded="true">
                                            ...
                                            @if($property['bedrooms'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.bedrooms') }}</small><strong>{{ $property['bedrooms'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['bathrooms'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.bathrooms') }}</small><strong>{{ $property['bathrooms'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['rooms'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.rooms') }}</small><strong>{{ $property['rooms'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['areaTotal'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.area') }}</small><strong>{{ $property['areaTotal'] }}
                                                                {{ __('property-single.meters') }}²</strong></div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['areaLiving'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.area_living') }}</small><strong>{{ $property['areaLiving'] }}
                                                                {{ __('property-single.meters') }}²</strong></div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['floor'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.floor') }}</small><strong>{{ $property['floor'] }}@if($property['floorsTotal'] ?? null)
                                                                / {{ $property['floorsTotal'] }}@endif</strong></div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['yearBuilt'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.year_built') }}</small><strong>{{ $property['yearBuilt'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['ceilingHeight'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.ceiling_height') }}</small><strong>{{ $property['ceilingHeight'] }}
                                                                {{ __('property-single.meters') }}</strong></div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['landArea'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.land_area') }}</small><strong>{{ $property['landArea'] }}
                                                                {{ __('property-single.meters') }}²</strong></div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['parkingSpaces'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.parking_spaces') }}</small><strong>{{ $property['parkingSpaces'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['pricePerSqm'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.price_per_sqm') }}</small><strong><x-price
                                                                    :amount="$property['pricePerSqm']"
                                                                    :currency="$property['currency'] ?? null" /></strong></div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['totalUnits'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.total_units') }}</small><strong>{{ $property['totalUnits'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['availableUnits'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.available_units') }}</small><strong>{{ $property['availableUnits'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['totalBuildings'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.total_buildings') }}</small><strong>{{ $property['totalBuildings'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['code'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.code') }}</small><strong>#{{ $property['code'] }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['renovationType'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.renovation_type') }}</small><strong>{{ __($enumKey('renovation', $property['renovationType'])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['constructionType'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.construction_type') }}</small><strong>{{ __($enumKey('construction', $property['constructionType'])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['furnitureType'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.furniture_type') }}</small><strong>{{ __($enumKey('furniture', $property['furnitureType'])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['balconyType'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.balcony_type') }}</small><strong>{{ __($enumKey('balcony', $property['balconyType'])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if($property['terraceType'] ?? null)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.terrace_type') }}</small><strong>{{ __($enumKey('terrace', $property['terraceType'])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            {{-- Tiles below show only single-value variants; multi-value sets render in the
                                            "additional_details" accordion below. --}}
                                            @if(!empty($property['heatingType']) && count((array) $property['heatingType']) === 1)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        <x-icon name="thermostat" class="text-primary" />
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.heating_type') }}</small><strong>{{ __($enumKey('heating', ((array) $property['heatingType'])[0])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if(!empty($property['parkingType']) && count((array) $property['parkingType']) === 1)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        <x-icon name="local_parking" class="text-primary" />
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.parking_type') }}</small><strong>{{ __($enumKey('parking', ((array) $property['parkingType'])[0])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if(!empty($property['windowView']) && count((array) $property['windowView']) === 1)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.window_view') }}</small><strong>{{ __($enumKey('view', ((array) $property['windowView'])[0])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if(!empty($property['zoningType']))
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.zoning_type') }}</small><strong>{{ __($enumKey('zoning', $property['zoningType'])) }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            @if(isset($property['sunDirection']) && $property['sunDirection'] !== null && $property['sunDirection'] !== false)
                                                <div class="col-12 col-sm-6 col-lg-4">
                                                    <div class="feature-item d-flex align-items-center gap-2">
                                                        ...
                                                        <div><small
                                                                class="text-muted d-block">{{ __('property-single.sun_direction') }}</small><strong>{{ $property['sunDirection'] ? __('property-single.yes') : __('property-single.no') }}</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                            ...
                            @if(!empty($multiBlocks))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-extra" aria-expanded="true">
                            ...
                            @if(!empty($boolFeatures))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-3" aria-expanded="true">
                            ...
                            @if(!empty($property['amenities']))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-4" aria-expanded="true">
                            ...
                            @if(!empty($property['features']))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-feat" aria-expanded="true">
                                        ...
                                        <div class="accordion-body">
                                            <div class="row row-gap-2">
                                                @foreach($property['features'] as $feat)
                                                    ...
                                                    <div class="col-12 col-md-6 col-xl-4">
                                                        <div class="feature-item d-flex align-items-center gap-2">
                                                            <x-icon :name="$featureIconMap[$fKey] ?? 'check_circle'"
                                                                class="text-success" />
                            ...
                            @if(!empty($property['appliances']))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-appl" aria-expanded="true">
                                        ...
                                        <div class="accordion-body">
                                            <div class="row row-gap-2">
                                                @foreach($property['appliances'] as $appl)
                                                    ...
                                                    <div class="col-12 col-md-6 col-xl-4">
                                                        <div class="feature-item d-flex align-items-center gap-2">
                                                            <x-icon :name="$applianceIconMap[$aKey] ?? 'devices'"
                                                                class="text-primary" />
                            ...
                            @if(!empty($property['utilities']))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-util" aria-expanded="true">
                                        ...
                                        <div class="accordion-body">
                                            <div class="row row-gap-2">
                                                @foreach($property['utilities'] as $util)
                                                    ...
                                                    <div class="col-12 col-md-6 col-xl-4">
                                                        <div class="feature-item d-flex align-items-center gap-2">
                                                            <x-icon :name="$utilityIconMap[$uKey] ?? 'power'"
                                                                class="text-warning" />
                            ...
                            @if($isRental && ($property['deposit'] ?? $property['petsPolicy'] ?? $property['utilitiesPolicy'] ?? null))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-cond" aria-expanded="true">
                                                ...
                                                @if($property['deposit'] ?? null)
                                                    <div class="col-12 col-sm-6 col-lg-4">
                                                        <div class="feature-item d-flex align-items-center gap-1">
                                                            ...
                                                            <div><small
                                                                    class="text-muted d-block">{{ __('property-single.deposit') }}</small><strong><x-price
                                                ...
                                                @if($property['utilitiesPolicy'] ?? null)
                                                    <div class="col-12 col-sm-6 col-lg-4">
                                                        <div class="feature-item d-flex align-items-center gap-1">
                                                            ...
                                                            <div><small
                                                                    class="text-muted d-block">{{ __('property-single.utilities_policy') }}</small><strong>{{ __($enumKey('utilities', $property['utilitiesPolicy'])) }}</strong>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endif
                                                @if($property['petsPolicy'] ?? null)
                                                    <div class="col-12 col-sm-6 col-lg-4">
                                                        <div class="feature-item d-flex align-items-center gap-1">
                                                            ...
                                                            <div><small
                                                                    class="text-muted d-block">{{ __('property-single.pets_policy') }}</small><strong>{{ __($enumKey('policy', $property['petsPolicy'])) }}</strong>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endif
                                                @if($property['childrenPolicy'] ?? null)
                                                    <div class="col-12 col-sm-6 col-lg-4">
                                                        <div class="feature-item d-flex align-items-center gap-1">
                                                            ...
                                                            <div><small
                                                                    class="text-muted d-block">{{ __('property-single.children_policy') }}</small><strong>{{ __($enumKey('policy', $property['childrenPolicy'])) }}</strong>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endif
                                                @if(!empty($property['isNegotiable']))
                                                    <div class="col-12 col-sm-6 col-lg-4">
                                                        <div class="feature-item d-flex align-items-center gap-1">
                                                            ...
                                                            <div><small
                                                                    class="text-muted d-block">{{ __('property-single.negotiable') }}</small><strong>{{ __('property-single.yes') }}</strong>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endif
                                                @if(!empty($property['isLongTermRental']))
                                                    <div class="col-12 col-sm-6 col-lg-4">
                                                        <div class="feature-item d-flex align-items-center gap-1">
                                                            ...
                                                            <div><small
                                                                    class="text-muted d-block">{{ __('property-single.long_term_rental') }}</small><strong>{{ __('property-single.yes') }}</strong>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endif
                            ...
                            @if(!empty($property['extendedAttributes']) && is_array($property['extendedAttributes']))
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-ext" aria-expanded="true">
                            ...
                            @if($priceHistory->isNotEmpty())
                                <div class="accordion-item">
                                    <div class="accordion-header">
                                        <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#accordion-hist" aria-expanded="true">
                                                ...
                                                @foreach($priceHistory as $entry)
                                                    ...
                                                    <li class="price-history-item d-flex align-items-start gap-3 pb-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div>{{ $label }}: <strong><x-price :amount="$newAmt"
                                                                        :currency="$cur" /></strong>
                                                                @if($oldAmt !== null) <span
                                                                    class="text-muted ms-1 text-decoration-line-through"><x-price
                                                                :amount="$oldAmt" :currency="$cur" /></span>@endif
                                                            </div>
                                                        </div>
                                                    </li>
                                                @endforeach
                    ...
                    <div class="col-xl-4 buy-details-item">
                        <div class="property-single-sidebar">
                            ...
                            <div class="card agent-card">
                                ...
                                <div class="card-body agent-card-body text-center">
                                        ...
                                        <div class="agent-ws-row mb-3">
                                            @if(!empty($workspace['logoUrl']))
                                                <img src="{{ $workspace['logoUrl'] }}" alt="{{ $workspace['name'] }}"
                                                    class="agent-ws-logo">
                                            @else
                                                <div class="agent-ws-initial">
                                                    {{ mb_strtoupper(mb_substr($workspace['name'], 0, 1)) }}</div>
                                            @endif
                                    ...
                                    @if($agentName)
                                        @if(!empty($workspace['name']))
                                        <hr class="my-3">@endif
                                        <div class="agent-avatar-circle mb-2">{{ mb_strtoupper(mb_substr($agentName, 0, 1)) }}
                                        </div>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered modal-sm">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title fw-bold mb-0">{{ __('property-single.contact_title') }}
                                                </h5>
                                                <button type="button" class="btn-close btn-close-modal custom-btn-close"
                                                    data-bs-dismiss="modal" aria-label="Close"><x-icon name="close" /></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="d-flex flex-column gap-2">
                                                    @if($cPhoneDial)
                                                        <a href="tel:+{{ $cPhoneDial }}"
                                                    ...
                                                    @if($cEmail)
                                                        <a href="mailto:{{ $cEmail }}"
                                                            ...
                                                            <span
                                                                class="d-inline-flex align-items-center justify-content-center flex-shrink-0"
                                                                style="width:38px;height:38px;border-radius:50%;background:#f0f0f0;color:#555"><x-icon
                                                                    name="mail" size="18" /></span>
                                ...
                                <div class="card-body">
                                    @if($property['propertyType'] ?? null)
                                        ...
                                        <div class="d-flex justify-content-between align-items-center py-2">
                                            ...
                                            <span
                                                class="fw-semibold">{{ __($__pdType) !== $__pdType ? __($__pdType) : $property['propertyType'] }}</span>
                                        </div>
                                    @endif
                                    @if($property['transactionType'] ?? null)
                                        ...
                                        <div class="d-flex justify-content-between align-items-center py-2">
                                            ...
                                            <span class="fw-semibold">
                                                ...
                                                @elseif(str_starts_with($__pdTx, 'rent'))
                                                    {{ __('property-single.rent_monthly') }}
                                                @else {{ __('property-single.for_sale') }}
                                                @endif
                                            </span>
                                        </div>
                                    ...
                                    @if($property['status'] ?? null)
                                        ...
                                        <div class="d-flex justify-content-between align-items-center py-2">
                                            ...
                                            <span>
                                                @if($__pdStatus === 'Draft')<span
                                                    class="badge bg-secondary">{{ __('property.status_draft') }}</span>
                                                @elseif($__pdStatus === 'Active')<span
                                                    class="badge bg-success">{{ __('property.status_active') }}</span>
                                                @elseif($__pdStatus === 'Sold')<span
                                                    class="badge bg-dark">{{ __('property.status_sold') }}</span>
                                                @elseif($__pdStatus === 'Rented')<span
                                                    class="badge bg-dark">{{ __('property.status_rented') }}</span>
                                                @elseif($__pdStatus === 'Reserved')<span class="badge"
                                                    style="background:#f59e0b">{{ __('property.status_reserved') }}</span>
                                                @elseif($__pdStatus === 'Inactive')<span
                                                    class="badge bg-secondary">{{ __('property.status_inactive') }}</span>
                            ...
                            {{-- Enquire — TEMPORARILY HIDDEN. To restore: change @if(false) below to @if(true) (or remove
                            the @if/@endif). --}}
                            @if(false)
                                <!-- Enquire -->
                                <div class="card">
                                        ...
                                        <form id="enquireForm" data-slug="{{ $property['slug'] ?? '' }}">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="name" class="form-control"
                                                    placeholder="{{ __('property-single.your_name') }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label fw-semibold">{{ __('property-single.email') }}</label>
                                                <input type="email" name="email" class="form-control"
                                                    placeholder="{{ __('property-single.your_email') }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label fw-semibold">{{ __('property-single.phone') }}</label>
                                                <input type="tel" name="phone" class="form-control"
                                                    placeholder="{{ __('property-single.your_phone') }}">
                                            </div>
                                            <div class="mb-4">
                                                <label
                                                    class="form-label fw-semibold">{{ __('property-single.description_label') }}</label>
                                                <textarea name="message" class="form-control" rows="3"
                                                    style="resize:none;overflow:hidden"
                                                    placeholder="{{ __('property-single.enquire_message_placeholder', ['title' => $property['title'] ?? '']) }}"></textarea>
                                            </div>
                            ...
                            @if(($property['country'] ?? null) || ($property['city'] ?? null) || ($property['street'] ?? null) || $__hasCoords)
                                    ...
                                    <div class="card-body">
                                        ...
                                        @if($property['street'] ?? null)
                                            <div class="d-flex justify-content-between align-items-center gap-4 py-2 border-bottom">
                                                ...
                                                @endphp
                                                <span
                                                    class="fw-semibold text-end fs-14">{{ $property['street'] }}{{ $__bn !== '' ? ', ' . $__bn : '' }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($__canMap)
                                            ...
                                            <div id="poiLoading" style="display:none; margin-top:16px;">
                                                <div class="d-flex align-items-center gap-2">
                                                    <span
                                                        class="fs-14 text-muted">{{ __('property-single.additional_info') }}</span>
                                                    <div class="spinner-border spinner-border-sm ms-1"
                                                        style="color:#03bd9d; width:14px; height:14px; border-width:2px;"
                                                        role="status"></div>
                                                </div>
                                            </div>
                                            <div id="poiAdditional" style="display:none; margin-top:16px;">
                                                <h6 class="mb-4 fs-14 text-muted">{{ __('property-single.additional_info') }}</h6>
                                                <div id="poiNearestList"></div>
                                            </div>

                                            {{-- Nearby: category tabs that toggle POI markers on the map below --}}
                                            <div id="poiTabs"
                                                style="display:none; margin-top:16px; border-top:1px solid var(--gray-100); padding-top:12px;">
                                                <h6 class="mb-4 fs-14 text-muted">{{ __('property-single.nearby') }}</h6>
                                                <div class="d-flex gap-2 poi-tabs-row">
                                                    <button type="button" class="btn btn-sm poi-tab flex-shrink-0"
                                                        data-cat="transport">{{ __('property-single.cat_transport') }}</button>
                                                    <button type="button" class="btn btn-sm poi-tab flex-shrink-0"
                                                        data-cat="education">{{ __('property-single.cat_education') }}</button>
                                                    <button type="button" class="btn btn-sm poi-tab flex-shrink-0"
                                                        data-cat="food">{{ __('property-single.cat_food') }}</button>
                                                </div>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    @if($__canMap)
                                        ...
                                        <div class="custom-map position-relative">
                                            ...
                                            <div id="mapControlBtns" class="map-control-btns" style="display:none;">
                                                <button id="mapGoHome" class="map-control-btn map-control-btn--home"
                                                    title="{{ __('property-single.location') }}">
                                                    <x-icon name="home" />
                                                </button>
                                            </div>
                                ...
                                @if($__canMap)
                                    ...
                                    <div id="mapFullscreenOverlay"
                                        style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; z-index:99999; background:#000;">
                                        <div id="mapFullscreenJsMap" style="width:100%;height:100%;"></div>
                                    </div>
                                    <!-- Кнопка закрытия fullscreen — вне карты, всегда поверх -->
                                    <button id="mapFullscreenClose"
                                        style="display:none; position:fixed; top:12px; left:12px; z-index:2147483647; width:40px; height:40px; border:none; border-radius:6px; background:rgba(0,0,0,0.7); color:#fff; cursor:pointer; align-items:center; justify-content:center;">
                                        <x-icon name="fullscreen_exit" size="24" />
                                    </button>
                                    <!-- Category tabs over the fullscreen map -->
                                    <div id="mapFsTabs"
                                        style="display:none; position:fixed; bottom:12px; left:50%; transform:translateX(-50%); z-index:2147483647; gap:8px; background:rgba(0,0,0,0.55); padding:6px; border-radius:24px;">
                                        <button type="button" class="btn btn-sm poi-tab poi-tab-fs"
                                            data-cat="transport">{{ __('property-single.cat_transport') }}</button>
                                        <button type="button" class="btn btn-sm poi-tab poi-tab-fs"
                                            data-cat="education">{{ __('property-single.cat_education') }}</button>
                                        <button type="button" class="btn btn-sm poi-tab poi-tab-fs"
