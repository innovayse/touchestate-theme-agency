...
<header x-data="{ scrolled: false, mobile: false }" @scroll.window="scrolled = window.scrollY > 20"
    ...
    <div class="container-x flex h-20 items-center justify-between gap-4">
        ...
        <a href="{{ url('/' . $locale) }}" class="flex shrink-0 items-center gap-2.5">
            ...
            @else
                <svg width="30" height="24" viewBox="0 0 30 24" fill="none" class="text-ink">
                    <path d="M2 22 L11 4 L16.5 13.5 L20 8 L28 22 Z" stroke="currentColor" stroke-width="1.6"
                        stroke-linejoin="round" />
                </svg>
            @endif
            <span
                class="font-display text-xl font-bold tracking-[0.06em] text-ink">{{ $workspace['name'] ?? 'GOLDHOUSE' }}</span>
        </a>
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ url('/' . $locale . '/favorites') }}" title="{{ __('header.favorites') }}"
                ...
                x-data="{ count: 0 }" x-init="count = (JSON.parse(localStorage.getItem('te_favorites') || '[]')).length"
            ...
            <a href="{{ url('/' . $locale . '/compare') }}" title="{{ __('header.compare') }}"
                ...
                x-data="{ count: 0 }" x-init="count = (JSON.parse(localStorage.getItem('te_compare') || '[]')).length"
                @te-storage.window="count = (JSON.parse(localStorage.getItem('te_compare') || '[]')).length">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    :stroke-width="count > 0 ? '2.5' : '1.8'">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.outside="open = false"
                    class="grid h-10 w-10 place-items-center justify-center rounded-full border border-sand bg-panel text-sm font-medium text-ink transition hover:border-brand-400">
                    <span x-text="{'USD':'$','EUR':'€','AMD':'֏','RUB':'₽'}[$store.fx.currency] ?? '$'"
                        class="text-base font-semibold">$</span>
                </button>
                <div x-show="open" x-transition x-cloak
                    class="absolute right-0 mt-2 w-32 overflow-hidden rounded-xl border border-sand bg-white py-1 shadow-lg">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" @click.outside="open = false"
                    ...
                    <svg width="12" height="12" viewBox="0 0 12 8" fill="none" stroke="currentColor" stroke-width="1.5"
                        :class="open?'rotate-180':''">
                        <path d="M1 1l5 5 5-5" />
                    </svg>
                </button>
                <div x-show="open" x-transition x-cloak
                    class="absolute right-0 mt-2 w-44 overflow-hidden rounded-xl border border-sand bg-white py-1 shadow-lg">
            ...
            <button @click="mobile = true"
                class="grid h-10 w-10 place-items-center rounded-full border border-sand bg-panel text-ink lg:hidden">
    ...
    {{-- ── Mobile modal ────────────────────────────────── --}}
    {{-- Backdrop --}}
    <div x-show="mobile" x-transition:enter="transition-opacity ease-out duration-200"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="transition-opacity ease-in duration-150" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" x-cloak @click="mobile = false"
    ...
    <div x-show="mobile" x-transition:enter="transition ease-out duration-300"
        ...
        x-transition:leave-end="translate-x-full" x-cloak @click.outside="mobile = false"
        ...
        <div class="flex h-20 shrink-0 items-center justify-between border-b border-sand px-6">
            <a href="{{ url('/' . $locale) }}" class="flex items-center gap-2.5" @click="mobile = false">
                ...
                @endif
                <span
                    class="font-display text-lg font-bold tracking-[0.06em] text-ink">{{ $workspace['name'] ?? 'GOLDHOUSE' }}</span>
            </a>
        ...
        <nav class="flex flex-1 flex-col overflow-y-auto px-6 py-6">
            @foreach($navLinks as $link)
                <a href="{{ $link['href'] }}" @click="mobile = false" class="flex items-center border-b border-sand/60 py-4 text-base font-semibold transition
                              {{ $link['match']() ? 'text-brand-600' : 'text-ink hover:text-brand-600' }}">
                    {{ $link['label'] }}
                    <svg class="ml-auto shrink-0 {{ $link['match']() ? 'text-brand-600' : 'text-neutral-300' }}" width="16"
        ...
        {{-- Bottom bar: icons --}}
        <div class="shrink-0 border-t border-sand px-6 py-5 flex items-center">
